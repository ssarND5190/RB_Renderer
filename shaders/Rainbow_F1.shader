Shader "Custom/Rainbow_F1"
{
    Properties
    {
        ...
        _Width("Specular Width", Range(1,100)) = 20
        _Fallof("Specular Fallof", Range(-100,100)) = 0
    }
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 frag (v2f i) : SV_Target
            {
                ...
                float NdotH = max(dot(normalize(i.normal), halfwayDir), 0.0);
                float specularIntensity = pow(NdotH, _Width) * exp(_Fallof * NdotH);
                color += _SpecularColor * specularIntensity * shadow;
